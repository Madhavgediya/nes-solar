  ...
  const getPages = (dir: string) => {
    ...
    files.forEach((file) => {
      ...
      if (fs.statSync(filePath).isDirectory()) {
        if (file === "api") return; // Skip API folder
        getPages(filePath); // Recursively get pages in subdirectories
      } else {
...
// Simulate fetching dynamic blog posts (replace with real data fetching)
